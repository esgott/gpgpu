...
// You can remove CL_DEVICE_TYPE_ALL if you have proper GPU ;) I don't
...
void printTimeStats(cl_event event) {
    ...
    std::cout << "[start] " << execStart << " [end] " << execEnd << " [time] " << (execEnd - execStart) / 1e+06 << "ms."
            << std::endl;
}
...
void map() {
    ...
    clEnqueueNDRangeKernel(cl.cqueue(), mapKernel, 1, NULL, &workSize, NULL, 0, NULL, &kernelEvent);
    ...
    for (size_t i = 0; i < dataSize; ++i) {
        if (hData[i] != (float) i * (float) i) {
            std::cerr << "Wrong value at " << i << ". Value is " << hData[i] << " but expected "
                    << (float) i * (float) i << "." << std::endl;
            break;
        }
    }
...
void histogram_global() {
    ...
    clEnqueueNDRangeKernel(cl.cqueue(), histogramGlobalKernel, 1, NULL, &workSize, NULL, 0, NULL, &kernelEvent);
    ...
    for (size_t i = 0; i < histogramSize; ++i) {
        if (refHist[i] != hHist[i]) {
            std::cerr << "Wrong value at " << i << ". Value is " << hHist[i] << " but expected " << refHist[i] << "."
                    << std::endl;
            break;
        }
    }
...
void histogram_local() {
    ...
    cl_event kernelEvent;
    const size_t workSize = dataSize;
    clEnqueueNDRangeKernel(cl.cqueue(), histogramLocalKernel, 1, NULL, &workSize, NULL, 0, NULL, &kernelEvent);
...
void reduce_global() {
    ...
    size_t workSize = dataSize;
    cl_event kernelEvent;
    clEnqueueNDRangeKernel(cl.cqueue(), reduceKernel, 1, NULL, &workSize, NULL, 0, NULL, &kernelEvent);
...
void exclusive_scan() {
    ...
    size_t workSize = dataSize;
    cl_event kernelEvent;
    clEnqueueNDRangeKernel(cl.cqueue(), exscanKernel, 1, NULL, &workSize, NULL, 0, NULL, &kernelEvent);
    ...
    for (size_t i = 0; i < dataSize; ++i) {
        if (oData[i] != hData[i]) {
            std::cerr << "Wrong value at " << i << ". Value is " << hData[i] << " but expected " << oData[i] << "."
                    << std::endl;
            break;
        }
    }
...
void compact() {
    ...
    size_t workSize = dataSize;
    cl_event kernelEvent;
    clEnqueueNDRangeKernel(cl.cqueue(), predicateKernel, 1, NULL, &workSize, NULL, 0, NULL, &kernelEvent);
    ...
    clEnqueueNDRangeKernel(cl.cqueue(), compactKernel, 1, NULL, &workSize, NULL, 0, NULL, &kernelEvent);
...
int main(int argc, char* argv[]) {
    clProgram = cl.createProgram("../src/lab8/kernels/programs.cl");
    map();
    histogram_global();
    histogram_local();
    reduce_global();
