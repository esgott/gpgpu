...
void testUniform1DArray(size_t maxWorkGroupSize, int randomNums, cl_mem randomsGPU) {
    ...
    CL_SAFE_CALL(
            clGetKernelWorkGroupInfo(testUniform1DKernel, cl.device_id(), CL_KERNEL_WORK_GROUP_SIZE, sizeof(workGroupSize), &workGroupSize, NULL));
    ...
    cl_mem bucketsGPU = clCreateBuffer(cl.context(), CL_MEM_READ_WRITE, sizeof(int) * workGroupSize * bucketNum, NULL,
            NULL);
    ...
    CL_SAFE_CALL(clEnqueueNDRangeKernel(cl.cqueue(), testUniform1DKernel, 1, NULL, &workGroupSize, NULL, 0, NULL, NULL));

    clFinish(cl.cqueue());

    CL_SAFE_CALL(
            clEnqueueReadBuffer(cl.cqueue(), bucketsGPU, CL_TRUE, 0, sizeof(int) * workGroupSize * bucketNum, buckets, 0, NULL, NULL));
...
void simpleLCG() {
    ...
    size_t workGroupSize = 0;
    CL_SAFE_CALL(
            clGetKernelWorkGroupInfo(randomLCGKernel, cl.device_id(), CL_KERNEL_WORK_GROUP_SIZE, sizeof(workGroupSize), &workGroupSize, NULL));
    ...
    CL_SAFE_CALL(
            clEnqueueWriteBuffer(cl.cqueue(), seedGPU, CL_FALSE, 0, sizeof(float) * workGroupSize, seedCPU, 0, NULL, NULL));

    float* randomsCPU = new float[workGroupSize * randomNumbers];
    cl_mem randomsGPU = clCreateBuffer(cl.context(), CL_MEM_READ_WRITE, sizeof(float) * workGroupSize * randomNumbers,
            NULL, NULL);
    ...
    CL_SAFE_CALL(clEnqueueNDRangeKernel(cl.cqueue(), randomLCGKernel, 1, NULL, &workGroupSize, NULL, 0, NULL, NULL));

    clFinish(cl.cqueue());

    CL_SAFE_CALL(
            clEnqueueReadBuffer(cl.cqueue(), randomsGPU, CL_TRUE, 0, sizeof(float) * workGroupSize * randomNumbers, randomsCPU, 0, NULL, NULL));
...
void simpleLFG() {
    ...
    size_t workGroupSize = 0;
    CL_SAFE_CALL(
            clGetKernelWorkGroupInfo(randomLFGKernel, cl.device_id(), CL_KERNEL_WORK_GROUP_SIZE, sizeof(workGroupSize), &workGroupSize, NULL));
    ...
    cl_mem randomStateGPU = clCreateBuffer(cl.context(), CL_MEM_READ_WRITE,
            sizeof(cl_uint) * workGroupSize * randomsStateSize, NULL, NULL);
    ...
    CL_SAFE_CALL(clEnqueueNDRangeKernel(cl.cqueue(), randomLFGKernel, 1, NULL, &workGroupSize, NULL, 0, NULL, NULL));
...
void simpleCTG() {
    ...
    size_t workGroupSize = 0;
    CL_SAFE_CALL(
            clGetKernelWorkGroupInfo(randomCTGKernel, cl.device_id(), CL_KERNEL_WORK_GROUP_SIZE, sizeof(workGroupSize), &workGroupSize, NULL));
    ...
    CL_SAFE_CALL(
            clEnqueueWriteBuffer(cl.cqueue(), seedGPU, CL_FALSE, 0, sizeof(cl_float2) * workGroupSize, seedCPU, 0, NULL, NULL));
    ...
    CL_SAFE_CALL(clEnqueueNDRangeKernel(cl.cqueue(), randomCTGKernel, 1, NULL, &workGroupSize, NULL, 0, NULL, NULL));
...
void hybridRNG() {
    ...
    size_t workGroupSize = 0;
    CL_SAFE_CALL(
            clGetKernelWorkGroupInfo(hybridRNGKernel, cl.device_id(), CL_KERNEL_WORK_GROUP_SIZE, sizeof(workGroupSize), &workGroupSize, NULL));
    ...
    CL_SAFE_CALL(
            clEnqueueWriteBuffer(cl.cqueue(), seedGPU, CL_FALSE, 0, sizeof(float) * workGroupSize * 4, seedCPU, 0, NULL, NULL));
    ...
    CL_SAFE_CALL(clEnqueueNDRangeKernel(cl.cqueue(), hybridRNGKernel, 1, NULL, &workGroupSize, NULL, 0, NULL, NULL));
...
void mersenneTwister() {
    mtProgram = cl.createProgram("../src/lab10/kernels/mersenneTwister.cl");
    ...
    const char* mtDataPath = "../src/lab10/kernels/mersenneTwister.dat";
    ...
    CL_SAFE_CALL(
            clEnqueueWriteBuffer(cl.cqueue(), d_MT, CL_TRUE, 0, sizeof(mt_struct_stripped) * MT_RNG_COUNT, h_MT, 0, NULL, NULL));
    ...
    CL_SAFE_CALL(clEnqueueNDRangeKernel(cl.cqueue(), mtKernel, 1, NULL, globalWorkSize, localWorksSize, 0, NULL, NULL));

    clFinish(cl.cqueue());

    float* h_RandGPU = new float[nRand];
    CL_SAFE_CALL(
            clEnqueueReadBuffer(cl.cqueue(), d_Rand, CL_TRUE, 0, sizeof(cl_float) * nRand, h_RandGPU, 0, NULL, NULL));
...
void haltonSequence() {
    ...
    size_t workGroupSize = 0;
    CL_SAFE_CALL(
            clGetKernelWorkGroupInfo(haltonKernel, cl.device_id(), CL_KERNEL_WORK_GROUP_SIZE, sizeof(workGroupSize), &workGroupSize, NULL));
    ...
    CL_SAFE_CALL(clEnqueueNDRangeKernel(cl.cqueue(), haltonKernel, 1, NULL, &workGroupSize, NULL, 0, NULL, NULL));
...
void mcInt1D() {
    ...
    size_t workGroupSize = 0;
    CL_SAFE_CALL(
            clGetKernelWorkGroupInfo(mcInt1DKernel, cl.device_id(), CL_KERNEL_WORK_GROUP_SIZE, sizeof(workGroupSize), &workGroupSize, NULL));
    ...
    CL_SAFE_CALL(
            clEnqueueWriteBuffer(cl.cqueue(), seedGPU, CL_FALSE, 0, sizeof(cl_float4) * workGroupSize, seedCPU, 0, NULL, NULL));
    ...
    CL_SAFE_CALL(clEnqueueNDRangeKernel(cl.cqueue(), mcInt1DKernel, 1, NULL, &workGroupSize, NULL, 0, NULL, NULL));

    clFinish(cl.cqueue());

    CL_SAFE_CALL(
            clEnqueueReadBuffer(cl.cqueue(), partialIntGPU, CL_TRUE, 0, sizeof(float) * workGroupSize, partialIntCPU, 0, NULL, NULL));
...
// TODO
// Implement a 3D Monte Carlo integrator
void mcInt3D() {

}

// TODO
// Implement a Black-Scholes differential equation solver
void blackscholes() {
...
int main(int arc, char* argv[]) {
    // OpenCL init
    program = cl.createProgram("../src/lab10/kernels/programs.cl");
